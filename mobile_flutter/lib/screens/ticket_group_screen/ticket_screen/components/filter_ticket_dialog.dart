...
class FilterTicketDialog extends StatefulWidget {
  const FilterTicketDialog({
    super.key,
  });

  @override
  State<FilterTicketDialog> createState() => _FilterTicketDialogState();
}

class _FilterTicketDialogState extends State<FilterTicketDialog> {
  @override
  void initState() {
    super.initState();
...
class FilterByOrganization extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TicketBloc, TicketState>(
      buildWhen: (p, c) =>
          p.organizationList != c.organizationList ||
        ...
        return DropdownMenuButtonFuction(
          ...
          onSelectionItem: (p0) {
            if (p0.object.id != state.selectedOrganization?.id) {
              context.ticketBloc.add(ChangeOrganizationEvent(model: p0.object));
            }
          },
        );
      },
      ...
      builder: (context, state) {
        return (state.selectedDepartment?.id != null &&
            ...
            ? DropdownMenuButtonFuction(
                ...
                onClickIcon: (p0) {},
                onSelectionItem: (p0) {
                  context.ticketBloc.add(ChangeTeamEvent(model: p0.object));
                },
              )
            : const SizedBox.shrink();
      },
            ...
            ? DropdownMenuButtonFuction(
                ...
                onSelectionItem: (p0) {
                  if (p0.object.id != state.selectedDepartment?.id) {
                    ...
                    context.ticketBloc.add(ChangeTeamEvent(model: TeamModel()));
                  }
                },
              )
            : const SizedBox.shrink();
