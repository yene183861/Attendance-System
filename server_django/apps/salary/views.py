...
from apps.salary.serializers import PayrollSerializer, CreatePayrollSerializer, UpdatePayrollSerializer


@method_decorator(name='update', decorator=swagger_auto_schema(auto_schema=None))
class PayrollViewSet(GenericViewSet):
    serializer_class = PayrollSerializer
    queryset = Payroll.objects.filter(deleted_at__isnull=True).order_by('-id')
    parser_classes = [JSONParser]
    action_serializers = {
        'create_request': CreatePayrollSerializer,
        'partial_update_request': UpdatePayrollSerializer,
    }

    permission_action_classes = {
        'create': [IsAuthenticated],
        'list': [IsAuthenticated],
        'retrieve': [(IsCeoOrDirectorUser & PermissionPayroll) | PermissionOwnerPayroll],
        'partial_update': [IsCeoOrDirectorUser & PermissionPayroll],
        'destroy': [IsCeoOrDirectorUser & PermissionPayroll]
    }
